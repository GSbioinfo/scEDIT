def run(inoutDict):
    """
    InDel mode is for analysis of CRISPR indels in single cell data from Tapestri
    ...
    samplesheetname=inoutDict["samples"]#sys.argv[1]
    run_dir=inoutDict["outDir"]#sys.argv[2]
    print(samplesheetname)
    script_dir = inoutDict["scriptdir"]+"/"#os.path.dirname(sys.argv[0]) #os.path.realpath(__file__)
    print("directoy of the python script:")
    print(script_dir)
    bin_dir = inoutDict["binDir"]
    ...
    if 'AnalysisStatus' in  run_import_df.columns:
        run_import_df=run_import_df.loc[run_import_df.AnalysisStatus=='Y']
    
    if 'gRNA_AMP_details' not in run_import_df.columns:
        print('Missing sgRNA amplicon details file location')
        sys.exit(0)
    print(run_import_df)

    print("*******************************************************************")
    print("Warning: scEDIT BaseEDit Version 001 runs multiple cppinputfile.\n")
    ...
    for subID in subjectID_list:
        ...
        for sample in sample_df["SampleName"].to_list():
            ...
            sgRNAFile = file_location["gRNA_AMP_details"][0] #"/home/gajendra/Dropbox/Tapestri_workflow/PRJNA889818/gRNA_AMP_details.xlsx"
            ...
            fulseq=[]
            file_list=[]
            #runinput_dir="/home/gajendra/Dropbox/Tapestri_workflow/PRJNA889818/temp_dir"
            amp_panel = amps_df["Custom_Panel"][0]
            ...
            sgrna_df=pd.read_excel(sgRNAFile)
            windowseq = []
            sgrnaseq =[]
            nonsg_seq = []
            sgrna_fileHeader = sgrna_df.columns
            cpp_sgrna = ''
            if not sgrna_df.empty :
                cpp_sgrna = '-g'  

            for sgrna in sgrna_df.itertuples():
                sg_seq = sgrna.gRNASeq 
                if sgrna.Strand <0:
                    direcn = 'N'
                else:
                    direcn = 'P'
                target= sgrna.Target
                win_seq = sgrna.gRNASeq 
                seqid = target+'_'+sgrna.Custom_Panel+':'+str(sgrna.AmpStart-1)+'-'+str(sgrna.AmpEnd)+'!'+sgrna.AMP_Edite+'$'+sgrna.gRNAID
                seqidwin = sgrna.AMP_gRNA_ID
                sgstart = sgrna.editLocF
                sgend = sgrna.editLocR
                
                seqid = seqid + ":"+str(sgstart)+"-"+str(sgend)+'|'+ direcn +'|'+str(sg_minIden)
                sgrna_rec=SeqRecord(Seq(sg_seq),id=seqid,description="",name=sgrna.gRNAID,dbxrefs=[]) 
                sgrnaseq.append(sgrna_rec)
                windowseq.append(sgrna_rec)
            ...
            if nsplit < 2:
                if cpp_sgrna == '-g':
                    cmd = bin_dir+'/./singleCell_BaseEdit.out -f $infile'+' -t '+ str(file_location['Numb_Threads'][0])+ ' -s no' + ' -g yes'
                else:
                    cmd = bin_dir+'/./tapripper -f $infile'+' -t '+ str(file_location['Numb_Threads'][0])+ ' -s no' + ' -g no'
            else:
                if cpp_sgrna == '-g':
                    cmd = bin_dir+'/./singleCell_BaseEdit.out -f $infile'+' -t '+ str(file_location['Numb_Threads'][0])+ ' -s yes' + ' -g yes'
                else:
                    cmd = bin_dir+'/./tapripper -f $infile'+' -t '+ str(file_location['Numb_Threads'][0])+ ' -s yes' + ' -g no'
            ...
            bash_cmds.append('set -e')
            bash_cmds.append('conda activate scEDIT')
            ...
            bash_cmds.append('done')
            bash_cmds.append('ipython3 '+script_dir+'/count_barcode_edit_correct.py '+result_dir+'/'+'intermit_csv/ '+ subID+'_'+sample+' '+'BaseEdit')
