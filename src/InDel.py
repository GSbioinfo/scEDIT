def run(inoutDict):
    ...
    for subID in subjectID_list:
        ...
        for sample in sample_df["SampleName"].to_list():
            ...
            if nsplit < 2:
                if cpp_sgrna == '-g':
                    cmd = bin_dir+'/./singleCell_HLWFA.out -f $infile'+' -t '+ str(file_location['Numb_Threads'][0])+ ' -s no' + ' -g yes'
                else:
                    cmd = bin_dir+'/./tapripper -f $infile'+' -t '+ str(file_location['Numb_Threads'][0])+ ' -s no' + ' -g no'
            else:
                if cpp_sgrna == '-g':
                    cmd = bin_dir+'/./singleCell_HLWFA.out -f $infile'+' -t '+ str(file_location['Numb_Threads'][0])+ ' -s yes' + ' -g yes'
                else:
            ...
            bash_cmds.append('done')
            bash_cmds.append('ipython3 '+script_dir+'/count_barcode_edit_correct.py '+result_dir+'/'+'intermit_csv/ '+ subID+'_'+sample+' '+'InDel')
