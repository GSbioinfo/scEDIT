...
for subID in subjectID_list:
    ...
    for sample in sample_df["SampleName"].to_list():
        ...
        myDirectoryList=['fastqs/','dump_fastqs/','intermit_csv/','final_count/', 'bamFiles/']
        ...
        for sgrna in sgrna_df.itertuples():
            ...
            for flseq in fulseq:
                ...
                if win_seq in flseq.seq:
                    ...
                    windirect = 'For'
                    seqidwin = flseq.name+":"+windirect+":"+str(winstart)+"-"+str(winend)+'$'+re.sub('\|.*','',flseq.id)+'|'+str(sg_minIden)
                ...
                if sg_seq in flseq.seq:
                    ...
                    sgdirect = 'For'
                    seqid = flseq.name+":"+sgdirect+":"+str(sgstart)+"-"+str(sgend)+'$'+re.sub('\|.*','',flseq.id)+'|'+str(sg_minIden)  
        ...
        if nsplit < 2:
            if cpp_sgrna == '-g':
                cmd = script_dir+'/./singleCell_edit_detector_Report.out -f $infile'+' -t '+ str(file_location['Numb_Threads'][0])+ ' -s no' + ' -g yes'
            else:
                cmd = script_dir+'/./tapripper -f $infile'+' -t '+ str(file_location['Numb_Threads'][0])+ ' -s no' + ' -g no'
        else:
            if cpp_sgrna == '-g':
                cmd = script_dir+'/./singleCell_edit_detector_Report.out -f $infile'+' -t '+ str(file_location['Numb_Threads'][0])+ ' -s yes' + ' -g yes'
            else:
        ...
        bash_cmds.append('#sh bash_saminfo.sh maininput_file')
